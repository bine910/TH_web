...
import { Typography, Card, CardHeader, CardMedia, CardContent, Divider, Button } from "@mui/material";
    ...
    useEffect(() => {
        ...
        fetchModel(`/user/${userId}`)
            .then(res => {
                if (isMounted) {
                    setUser(res.data);
                    if (setTopBarContext) setTopBarContext(`Photos of ${res.data.first_name} ${res.data.last_name}`);
                }
            })
            .catch(err => console.error("Error fetching user data in photos:", err));
            ...
            {advancedFeature ? (
                <>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px' }}>
                        <Button variant="contained" disabled={currentIndex === 0} onClick={goPrev}>
                            Previous
                        </Button>
                        <Button variant="contained" disabled={currentIndex === photos.length - 1} onClick={goNext}>
                            Next
                        </Button>
                    </div>
                </>
